...
(function (window) {
  function initProgressBar() {
    ...
    var createCheckPoints = function(stages, stageWidth, currentStageIndex) {
      var ul = createElement('ul', 'progress-barX', {padding:0}, '');
      var animationDelay = renderingWaitDelay;
      for (var index = 0; index < stages.length; index++) {
        let className = 'section'
        if(index==(stages.length-1) && stages[index].status) {
          className+=' '+stages[index].status
        }
        var li = createElement('li', className, {width: stageWidth + '%'}, stages[index].text);
        var span = createElement('span','stageDate',{} , stages[index].date)
        li.appendChild(span)
    ...
    var createHTML = function (wrapper, stages, currentStage) {
      ...
      var currentStageIndex = stages.findIndex(stage=>{
        return stage.text == currentStage
      });
      //create status bar
    ...
    var validateParameters = function(stages, currentStage, container) {
      if(!(typeof stages === 'object' && stages.length)) {
